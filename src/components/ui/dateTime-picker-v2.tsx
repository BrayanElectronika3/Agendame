...
export const DateTimePicker = forwardRef<HTMLButtonElement, DateTimePickerProps>(({ value, onChange }, ref) => {
  ...
  const handleDateChange = (selectedDate: Date | undefined) => {
    ...
    const updatedDate = moment(selectedDate).set({ hour: dateTime ? moment(dateTime).hour() : 0, minute: dateTime ? moment(dateTime).minute() : 0, }).toDate()
    setDateTime(updatedDate)
    onChange(updatedDate)
  }

  const handleTimeChange = (time: { hour: number; minute: number }) => {
    const updatedDate = moment(dateTime || new Date()).set({ hour: time.hour, minute: time.minute }).toDate()
    setDateTime(updatedDate)
    onChange(updatedDate)
  }
    ...
    <Popover>
      ...
      <PopoverContent className="w-auto">
        ...
        <div className="flex items-center space-x-2">
          ...
          <TimePicker
            interval={30}
            selectedTime={ dateTime ? { hour: moment(dateTime).hour(), minute: moment(dateTime).minute() } : undefined }
